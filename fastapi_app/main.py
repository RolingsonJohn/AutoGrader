    ...
    try:
        return HealthCheckResponse(
            status="healthy", version="2.0.0", services={
                "api": {
                    "healthy": True, "response_time_ms": 0}, "models": {
                    "healthy": True, "available_models": [
                        "ollama", "groq", "genai"]}})
...
async def listall():
    ...
    try:
        models = ollama.list()
        logger.info(
            f"Found {len(models.models) if hasattr(models, 'models') else 0} models")
        return models
    except Exception as e:
...
@app.post("/evaluate", response_model=SuccessResponse,
...
async def evaluate(
    ...
    """
    logger.info(
        f"Evaluation request for task {task_id}, theme {theme}, lang {prog_lang}")
    ...
    try:
        ...
        logger.info(
            f"Queuing evaluation task {task_id} for background processing")
        if background_tasks:
...
async def populate_rag(request: PopulateRagRequest):
    ...
    try:
        ...
        rag_service = RagService.get_instance(
            request.theme, settings.resources_dir)

        # Populate RAG
        rag_service.populate({"examples": request.examples})

        logger.info(
            f"RAG populated for theme {request.theme} with {len(request.examples)} examples")
        return SuccessResponse(
...
async def delete_example(request: DeleteRagRequest):
    ...
    """
    logger.info(
        f"Deleting example task {
            request.task_id} from theme {
            request.theme}")
