...
class CodeClassifier:
    ...
    def get_ngram_embedding(self, code: str) -> np.ndarray:
        if not self.vocab_ready:
            raise ValueError(
                "Vectorizer no entrenado. Llama a `fit_ngram_vocab()` primero.")
        vector = self.vectorizer.transform([code]).toarray()[0]
        norm = np.linalg.norm(vector)
        return vector / norm if norm > 0 else vector

    def euclidean_distance(
        self, ref1, ref2) -> float: return util.euclidean_sim(ref1, ref2).item()

    def manhattan_distance(
        self, ref1, ref2) -> float: return util.manhattan_sim(ref1, ref2).item()

    def cosine_similitude(self,
                          ref1,
                          ref2) -> float: return util.pytorch_cos_sim(ref1,
                                                                      ref2).item()

    def entropy_calculus(self, code: str) -> float:
        if not code:
            return 0
