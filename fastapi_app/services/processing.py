...
def process_files(theme: str, prog_lang: str, model_name: str, agent: str, api_key: str, token: str, zip_path: Path, rubric_path: Path):
    ...
    try:
        ...
        logger.info(f"Pre-evaluation complete. Classified scripts: {len(scripts)}")
        
        if not scripts:
            logger.warning("No scripts passed classification. Aborting evaluation.")
            return None
        ...
        logger.error(f"Error en la evaluación de los ficheros: {str(e)}", exc_info=True)
    ...
    try:
        ...
        logger.error(f"Error en la emisión del feedback a estudiantes: {str(e)}", exc_info=True)
